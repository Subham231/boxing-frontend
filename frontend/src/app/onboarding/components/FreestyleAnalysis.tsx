          ...
          <motion.div key="inst" initial={{opacity:0,y:15}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-15}} className="flex flex-col flex-1 justify-between gap-3">
            <header className="text-left">
              ...
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-primary/10 border border-primary/30 text-primary text-[8px] font-black uppercase tracking-wider mb-2">
                <Zap className="w-3 h-3" /> 30-SECOND AI CALIBRATION
              </div>
              <h1 className="text-2xl sm:text-3xl font-black italic uppercase leading-[0.93] tracking-tighter text-white">
                Freestyle<br /><span className="text-primary">Punch Analysis</span>
              </h1>
              <p className="text-white/50 mt-2 text-xs leading-relaxed font-semibold">
                Our on-device computer vision scores your real punch speed, power, and body mechanics — everything runs locally, nothing is sent out.
              </p>
            </header>

            {/* Setup video */}
            <div className="rounded-3xl overflow-hidden border border-primary/20 bg-black relative">
              <video
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent px-3 py-2">
                <span className="text-[9px] font-black uppercase text-primary tracking-wider">45° ANGLE — OPTIMAL POSITION</span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              {[
                { n:'1', head:'Stand at 45° to Camera', body:'Position the phone so it sees your body at a diagonal. This gives the AI the best view of your arm extension, shoulder rotation, and footwork.' },
                { n:'2', head:'6–8 Feet Away', body:'Step back so your full upper body (head to waist) is visible. Closer = worse tracking.' },
                { n:'3', head:'Throw Combinations Freely', body:'Jabs, crosses, hooks or any mix — throw real punches at full speed for the full 30 seconds.' },
              ].map(s => (
                <div key={s.n} className="flex items-start gap-3 p-3.5 rounded-2xl bg-white/[0.03] border border-white/[0.06]">
                  <div className="w-9 h-9 rounded-xl bg-primary/10 border border-primary/30 flex items-center justify-center text-primary font-black text-sm shrink-0">{s.n}</div>
                  <div>
                    <span className="text-[10px] font-black uppercase text-white tracking-wide block">{s.head}</span>
                    <span className="text-[9px] text-white/50 font-semibold leading-snug block mt-0.5">{s.body}</span>
                  </div>
                </div>
              ))}
            </div>
