...
const StructuredProgram: React.FC = () => {
  ...
  return (
    <div className="flex flex-col min-h-full justify-between py-2 pb-20 sm:pb-24">
      ...
      <main className="flex-1 flex flex-col gap-1 my-auto">
        {WEEKS.map((w, i) => (
          <motion.div
            key={w.label}
          ...
          >
            <div className="glass-card p-3 sm:p-4 rounded-3xl border-white/5 bg-black/40 flex items-center gap-3 sm:gap-4">
              <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-2xl bg-primary/10 border border-primary/20 flex items-center justify-center shrink-0">
                <span className="text-xs font-black text-primary">{i + 1}</span>
              </div>
              <div className="flex-1">
                <span className="text-[9px] font-black text-white/40 uppercase tracking-wide block">{w.label}</span>
                <span className="text-xs sm:text-sm font-black text-white uppercase">{w.desc}</span>
                <div className="h-1.5 rounded-full bg-white/5 mt-1.5 sm:mt-2 overflow-hidden">
                  <motion.div
