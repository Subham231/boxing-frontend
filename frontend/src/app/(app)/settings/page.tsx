    ...
    <div className="flex flex-col gap-6 anim-fade-in relative pb-16">
      ...
      <div className="flex flex-col gap-3.5">
        ...
        <div className="flex flex-col gap-2">
          <a
            href={`/account/link-email${liveProfile?.email ? `?email=${encodeURIComponent(liveProfile.email)}` : ''}`}
            className="glass-card p-4 rounded-3xl border border-primary/20 bg-black/40 flex justify-between items-center hover:border-primary/40 transition-all"
          >
            <div className="flex items-center gap-4">
              <Mail className="w-4 h-4 text-primary" />
              <div className="flex flex-col">
                <span className="text-xs font-black uppercase text-white">
                  {liveProfile?.email ? 'LINKED EMAIL' : 'ADD & VERIFY EMAIL'}
                </span>
                <span className="text-[10px] font-semibold text-white/50">
                  {liveProfile?.email
                    ? `${liveProfile.email} (Tap to change)`
                    : 'Recommended — enable passwordless sign-in & secure your data'}
                </span>
              </div>
            </div>
