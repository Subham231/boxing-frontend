...
export function AppShell({ children }: AppShellProps) {
  ...
  const isLiveMatch = pathname.startsWith('/spar/match');
  ...
  return (
    <div className="relative min-h-screen bg-bg-dark text-white flex flex-col">
      ...
      <main className={`flex-1 w-full mx-auto px-4 pt-[calc(env(safe-area-inset-top,0px)+1.25rem)] flex flex-col ${isLiveMatch ? 'max-w-5xl pb-8' : 'max-w-md pb-[120px]'}`}>
        {children}
      </main>
